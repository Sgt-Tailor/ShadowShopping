package com.shadowblox.sw123.ShadowShopping.korikutils;
	...
	public void help(CommandSender sender, String[] args) {
			...
			for (Method m : this.getClass().getMethods()) {
				for (Annotation a : m.getAnnotations()) {
					if (a instanceof command && m.getName().equalsIgnoreCase(args[0])) {
						command c = (command) a;
						sender.sendMessage("[" + ((hasPerms(sender, c.permissions())) ? ChatColor.GREEN : ChatColor.RED) + m.getName() + ChatColor.GRAY + " subcommand Summary]");
		...
		for (Method m : this.getClass().getMethods()) {
			for (Annotation a : m.getAnnotations()) {
				if (a instanceof command) {
					command c = (command) a;
					sender.sendMessage(((hasPerms(sender, c.permissions())) ? ChatColor.GREEN : ChatColor.RED) + m.getName() + " " + c.usage() + ChatColor.GRAY + " - " + c.description());
				}
			}
		}
